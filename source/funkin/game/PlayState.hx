...
class PlayState extends MusicBeatState
{
	...
	/**
	 * Botplay variables: whether botplay is enabled and the on-screen text.
	 */
	...
	**/
	...
	public function botplayAutoHit():Void {
		...
		// choose hit window consistent with StrumLine.updateNote
		var hw:Float = Flags.USE_LEGACY_TIMING ? hitWindow : ratingManager.lastHitWindow;

		var possible:Array<Note> = [];
		// collect tappable (non-sustain) notes inside hit window
		if (pl.notes != null) {
			for (n in pl.notes.members) {
				...
				if (Math.abs(Conductor.songPosition - nn.strumTime) <= hw) possible.push(nn);
			}
		}
		...
		for (note in possible) {
			...
			// perform hit
			goodNoteHit(pl, note);
			if (note.strumID < pl.members.length && pl.members[note.strumID] != null) pl.members[note.strumID].press(Conductor.songPosition);

			// phone-note dodge: check noteStyle in extras if present
			var nstyle:Dynamic = null;
			if (note.extra != null) nstyle = note.extra.get("noteStyle");
			if (nstyle == "phone" || nstyle == "phone-alt") {
				var pChar:Character = playerType == OPPONENT ? dad : boyfriend;
				var oChar:Character = playerType == OPPONENT ? boyfriend : dad;
				if (pChar != null) {
					if (pChar.animation.exists("dodge")) pChar.playAnim("dodge", true);
					else if (pChar.animation.exists("hey")) pChar.playAnim("hey", true);
					else pChar.sing(note.strumID);
				}
				if (oChar != null) {
					if (oChar.animation.exists("singThrow")) oChar.playAnim("singThrow", true);
					else oChar.playAnim("singSmash", true);
				}
				if (gf != null && gf.animation.exists("cheer")) gf.playAnim("cheer", true);
			}
		}

		// handle sustain pieces: mark sustain pieces as hit when passed and press their strums while in window
		if (pl.notes != null) {
			for (n in pl.notes.members) {
				var nn:Note = cast(n, Note);
				if (nn == null) continue;
				...
				if (nn.wasGoodHit) continue;
				if (nn.avoid) continue;
				// if passed the sustain piece, register hit
				if (Conductor.songPosition >= nn.strumTime) {
					goodNoteHit(pl, nn);
					if (nn.strumID < pl.members.length && pl.members[nn.strumID] != null) pl.members[nn.strumID].press(Conductor.songPosition);
				}
				// also, hold the strum while within sustain active window (sustainParent exists on pieces)
				if (nn.sustainParent != null) {
					var start:Float = nn.sustainParent.strumTime;
					var end:Float = nn.sustainParent.strumTime + nn.sustainParent.sustainLength;
					if (Conductor.songPosition >= start && Conductor.songPosition <= end) {
						if (nn.strumID < pl.members.length && pl.members[nn.strumID] != null) pl.members[nn.strumID].press(Conductor.songPosition);
					}
				}
			}
